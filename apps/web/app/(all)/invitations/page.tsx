...
import { VegecoopIcon } from "@/components/common/vegecoop-icon";
        ...
        return updateUserProfile({ last_workspace_id: redirectWorkspace?.id })
          .then(() => {
            ...
            return fetchWorkspaces().then(() => {
              return router.push(`/${redirectWorkspace?.slug}`);
            });
          })
          .catch(() => {
            setToast({
    ...
    <AuthenticationWrapper>
      <div className="flex h-full flex-col gap-y-2 overflow-hidden sm:flex-row sm:gap-y-0">
        <div className="relative h-1/6 flex-shrink-0 sm:w-2/12 md:w-3/12 lg:w-1/5">
          ...
          >
            <VegecoopIcon className="h-9 w-auto text-primary" />
          </Link>
                  ...
                  {invitations.map((invitation) => {
                    ...
                    return (
                      <button
                        key={invitation.id}
                        type="button"
                        className={`flex w-full items-center gap-2 rounded-sm border px-3.5 py-5 text-left ${
